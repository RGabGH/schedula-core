...
export class DefaultPopupPlugin implements ISchedulaPlugin, ITaskPopup {
    ...
    refreshItem(item: any): void {
        ...
        (popup.querySelector('#default-popup-field-link') as HTMLInputElement).value = item.Link ?? '';
    }
                ...
                <div class="popup-container">
                    ...
                    <div class="popup-content">
                        ...
                        <div class="tabcontent active" id="scheduler-default-popup-tab-general">
                            ...
                            <div class="formgroup"><label>Link</label><input class="taskinput" id="default-popup-field-link" type="text" placeholder="Task ID or shared key"></div>
                        </div>
    ...
    private _populatePopup(popup: HTMLElement, item: any): void {
        ...
        newSave.addEventListener('click', () => {
            ...
            const linkVal = (popup.querySelector('#default-popup-field-link') as HTMLInputElement).value.trim();
            item.Link = linkVal || undefined;
            popup.style.display = 'none';
            core.refreshItem(item);
            const linksPlugin = core.getPlugin?.('links');
            if (linksPlugin && core.settings.drawLinks) linksPlugin.drawLinks();
            const notifPlugin = core.getPlugin?.('notification');
            if (notifPlugin) notifPlugin.notifySaved(item);
