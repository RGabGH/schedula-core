(() => {
  ...
  var DragDropPlugin = class {
      ...
      if (!popup) {
        ...
        popup.querySelectorAll(".tab-btn").forEach((btn) => {
          btn.addEventListener("click", () => {
            ...
            btn.classList.add("active");
            const tab = btn.dataset.tab;
            (_a = document.getElementById(`scheduler-popup-tab-${tab}`)) == null ? void 0 : _a.classList.add("active");
          });
        });
    ...
    _populatePopup(popup, item) {
      ...
      popup.querySelector("#scheduler-popup-title").textContent = item.Text || "Task";
      popup.querySelector("#popup-field-text").value = item.Text || "";
      popup.querySelector("#popup-field-desc").value = item.Description || "";
      popup.querySelector("#popup-field-from").value = fmt(item.From);
      popup.querySelector("#popup-field-to").value = fmt(item.To);
      popup.querySelector("#popup-field-color").value = item.Color1 || "#000000";
      popup.querySelector("#popup-field-completion").value = (_a = item.Completion) != null ? _a : "";
      popup.querySelector("#popup-field-ref").value = item.Reference || "";
      popup.querySelector("#popup-field-json").value = JSON.stringify(item, null, 2);
      const saveBtn = popup.querySelector("#popup-btn-save");
      const newSave = saveBtn.cloneNode(true);
      saveBtn.parentNode.replaceChild(newSave, saveBtn);
      newSave.addEventListener("click", () => {
        item.Text = popup.querySelector("#popup-field-text").value;
        item.Description = popup.querySelector("#popup-field-desc").value;
        item.Color1 = popup.querySelector("#popup-field-color").value;
        const comp = parseInt(popup.querySelector("#popup-field-completion").value);
        item.Completion = isNaN(comp) ? void 0 : comp;
        item.Reference = popup.querySelector("#popup-field-ref").value;
        popup.style.display = "none";
        core.init();
      });
